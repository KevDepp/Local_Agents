...
async function runTests() {
    ...
    try {
        ...
        // 5b. Ensure doc/TODO.md doesn't contain a parseable template placeholder task.
        // (Historically a "T-xxx_slug" checklist example could be mistaken for a real next task on rebase.)
        console.log('[TEST] Verify TODO.md template has no placeholder task');
        const todoPath = path.join(FIXTURE_CWD, 'doc', 'TODO.md');
        const todoText = fs.readFileSync(todoPath, 'utf8');
        if (todoText.includes('T-xxx_slug')) throw new Error('TODO.md contains placeholder task id T-xxx_slug');
        if (todoText.includes('developer_codex|developer_antigravity')) throw new Error('TODO.md contains template pipe-owner placeholder');
        console.log('  -> PASS');
...
process.env.ANTIDEX_DATA_DIR = path.join(TEST_ROOT, '.antidex_data');
